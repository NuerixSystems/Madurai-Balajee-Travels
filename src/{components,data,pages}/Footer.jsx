...
const featuredChennai = chennaiAreas.slice(0, 6);
const featuredMadurai = maduraiAreas.slice(0, 6);

// Groups searched in order to find which city list the current area
// page's slug belongs to, so the footer can show OTHER nearby areas
// from that same list — different on every area page, instead of
// the same fixed 6+6 areas everywhere.
const AREA_GROUPS = [
      ...
      <div className="footer-main footer-main-wide">
        ...
        <div className="footer-col">
          ...
          <ul className="footer-link-list">
            ...
            <li><Link to="/home/local-tariff">Local Tariff</Link></li>
            <li><Link to="/home/occasions">Select For Various Occasions</Link></li>
        ...
        {nearby ? (
          ...
          <>
            ...
            <div className="footer-col">
              ...
              <p className="footer-other-cities-label">Popular Areas</p>
              <div className="footer-other-cities">
                {otherCities.map((c) => (
                  <Link to={`/ac-bus-rent-in/${c.slug}`} key={c.slug}>{c.name}</Link>
                ))}
              </div>
            </div>
          </>
